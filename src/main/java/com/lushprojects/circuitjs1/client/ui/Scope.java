...
import com.lushprojects.circuitjs1.client.elements.LogicOutputElm;
import com.lushprojects.circuitjs1.client.elements.OutputElm;
...
public class Scope {
    ...
    public boolean showI, showV, showScale, showMax, showMin, showFreq, lockScale, plot2d, plotXY, maxScale;
    public boolean showFFT, showNegative, showRMS;
	...
	if (val == 0) {
	    ...
	    if (ce != null && !(ce instanceof OutputElm || ce instanceof LogicOutputElm || ce instanceof AudioOutputElm
	...
	// toggle max scale. Why isn't this on by default? For the examples, we
	...
	// there is no UI for setting lockScale but it's used in some of the example
	// circuits (like crossover)
	...
	if (elm != null && elm instanceof TransistorElm) {
	    ...
	    sim.scopeRMSMenuItem.setTitle(canShowRMS() ? CirSim.LS("Show RMS Average") : CirSim.LS("Show Average"));
	...
	int flags = (showI ? 1 : 0) | (showV ? 2 : 0) | (showMax ? 0 : 4) | // showMax used to be always on
		...
		| (showScale ? 512 : 0) | (showFFT ? 1024 : 0) | (maxScale ? 8192 : 0) | (showRMS ? 16384 : 0);
	flags |= FLAG_PLOTS;
	int eno = sim.locateElm(elm);
	if (eno < 0)
	    return null;
	String x = "o " + eno + " " + vPlot.speed + " " + vPlot.value + " " + flags + " " + scale[UNITS_V] + " "
		+ scale[UNITS_A] + " " + position + " " + plots.size();
	int i;
	for (i = 0; i < plots.size(); i++) {
