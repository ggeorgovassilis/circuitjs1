...
public abstract class CompositeElm extends CircuitElm {
	...
	for (int i = 0; i < compElmList.size(); i++) {
	    ...
	    tstring = tstring.replaceFirst("[A-Za-z0-9]+_0_0_0_0_", ""); // remove unused tint_x1 y1 x2 y2 coords for
									 // internal components
	    dumpStr += " " + tstring;
	}
	...
	// In the meantime subclasses should override this if they know nodes are not
	// connected
	return true;
    }
    ...
    public abstract void getInfo(String arr[]);
    ...
    // It is hard to write a general purpose getCurrentIntoNode routine because this
    // is not defined
    // for many circuit elements. Working-around by using getCurrentIntoPoint
    // doesn't work for
    // internal components as they don't have points.
    // If all components in the composite have "getCurrentIntoNode" implemented then
    // you can use this
