...
public class OrGateElm extends GateElm {
	...
	if (useEuroGates()) {
	    ...
	    // 33-37 = left curve
	    Point triPoints[] = CircuitElementSupport.newPointArray(38);
		...
		double a = i / 16.;
		double b = 1 - a * a;
		CircuitElementSupport.interpPoint2(lead1, lead2, triPoints[i], triPoints[32 - i], .5 + a / 2, b * hs2);
	    }
	    double ww2 = (ww == 0) ? dn * 2 : ww * 2;
	    for (i = 0; i != 5; i++) {
		double a = (i - 2) / 2.;
		double b = 4 * (1 - a * a) - 2;
		CircuitElementSupport.interpPoint(lead1, lead2, triPoints[33 + i], b / (ww2), a * hs2);
		if (this instanceof XorGateElm)
		    linePoints[i] = CircuitElementSupport.interpPoint(lead1, lead2, (b - 5) / (ww2), a * hs2);
	    }
